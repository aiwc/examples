"""
Deep Q Neural Network class for AI Soccer
Example: 2 fully connected layers
...
class NeuralNetwork:
	def __init__(self, NetworkToCopy = None, RestoreFromFile = False, ReplayNetwork = False):
		...
		self.Q_theta = tf.matmul(out_layer1_input, self.W_ol1) + self.b_ol1  # No activation function in practice
		...
		self.train_step = tf.train.AdamOptimizer(lr).minimize(self.loss_fct)
		...
		if NetworkToCopy == None and RestoreFromFile == False:
			...
			if RestoreFromFile == True:
				self.saver = tf.train.Saver() # Restores saved network
				self.saver.restore(self.sess, "/home/aiwc/Desktop/test_world/examples/dqn_py/dqn.ckpt")
			elif RestoreFromFile != False:
	...
	def SaveToFile(self, filename = "/home/aiwc/Desktop/test_world/examples/dqn_py/dqn.ckpt"):
