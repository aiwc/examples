...
class NeuralNetwork:
	def __init__(self, NetworkToCopy = None, RestoreFromFile = False, ReplayNetwork = False):
		...
		self.phy = tf.compat.v1.placeholder(tf.float32, shape=[None, self.frame_res*self.nframes]) # Flattened last nf frames of the game
		self.y = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
		self.action = tf.compat.v1.placeholder(tf.float32, shape=[None, NumberOfActions])
		...
		self.train_step = tf.compat.v1.train.AdamOptimizer(learning_rate).minimize(self.loss_fct)

		if ReplayNetwork == True:
			self.update_ph_wfc1 = tf.compat.v1.placeholder(self.W_fc1.dtype, shape=[self.frame_res*self.nframes, fc1_num_neurons])
			self.update_ph_wfc2 = tf.compat.v1.placeholder(self.W_fc2.dtype, shape=[fc1_num_neurons, fc2_num_neurons])
			self.update_ph_bfc1 = tf.compat.v1.placeholder(self.b_fc1.dtype, shape=[fc1_num_neurons])
			self.update_ph_bfc2 = tf.compat.v1.placeholder(self.b_fc2.dtype, shape=[fc2_num_neurons])
			self.update_ph_wol1 = tf.compat.v1.placeholder(self.W_ol1.dtype, shape=[fc2_num_neurons, out_layer1_n_outputs])
			self.update_ph_bol1 = tf.compat.v1.placeholder(self.b_ol1.dtype, shape=[out_layer1_n_outputs])
		...
		if NetworkToCopy == None and RestoreFromFile == False:
			self.sess.run(tf.compat.v1.global_variables_initializer()) # Initializes Variables
