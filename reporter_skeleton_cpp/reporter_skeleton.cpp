// File:              reporter_skeleton.cpp
// Date:              Jan. 24, 2018
// Description:       A simple AI reporter
  ...
  void update(const aiwc::frame& f)
  {
    ...
    if(f.reset_reason == aiwc::GAME_END) {
      ...
      if(f.score[ATEAM] > f.score[BTEAM]) {
        paragraphs.emplace_back("A team won the game with score "
                            + std::to_string(f.score[ATEAM]) + ":" + std::to_string(f.score[BTEAM]));
      }
      else if(f.score[ATEAM] < f.score[BTEAM]) {
        paragraphs.emplace_back("B team won the game with score "
                            + std::to_string(f.score[ATEAM]) + ":" + std::to_string(f.score[BTEAM]));
      }
      else {
        paragraphs.emplace_back("No one won the game with score "
                            + std::to_string(f.score[ATEAM]) + ":" + std::to_string(f.score[BTEAM]));
      }

      paragraphs.emplace_back("I can't help but tell you the stories when I was in LA.. blahblahblah");
